...
if (nrow(dmps)>30){
	hist(dmps$slope, xlim = c(-0.4,0.4), xlab='DNAm difference between sexes', ylab='Frequency', main='DMPs')
		} else {
	hist(dmps$slope, xlab='DNAm difference between sexes', ylab='Frequency', main='DMPs')
}
...
bp=barplot(height=as.matrix(normalized_Relation_to_Island), xlab='Genomic Position', ylab='Normalized Frequency', main='DMPs')
...
for (i in 1:n){
	...
	boxplot(as.vector(beta[i,])~factor(pd$sex, levels=c('Female','Male')),ylab='DNAm(Beta)', xlab='Sex', main=c(zag1,zag2), cex.main=1, cex.lab=2, cex.axis=2, outline=FALSE)
	stripchart(as.vector(beta[i,])~factor(pd$sex, levels=c('Female','Male')), vertical = TRUE, method = "jitter", add = TRUE, pch = 21, col = 'black', bg='gray')
}
...
dx_short=as.vector(pd$Dx)
dx_short[which(dx_short=='Glioblastoma')]='GBM'
...
for (i in 1:n){
	zag1=paste0('probe ', probeFit$probe[i])
	zag2=paste0('p=',signif(probeFit$p.value[i],3), '; t=',signif(probeFit$t[i],3))
	boxplot(as.vector(beta[i,])~factor(xl, levels=c('F (Control)', 'M (Control)', 'F (GBM)', 'M (GBM)')),
		ylab='DNAm(Beta)', xlab=NULL, main=c(zag1,zag2), cex.main=1, cex.lab=2, cex.axis=1, outline=FALSE, at=c(1,2,4,5))
	stripchart(as.vector(beta[i,])~factor(xl, levels=c('F (Control)', 'M (Control)', 'F (GBM)', 'M (GBM)')), 
		vertical = TRUE, method = "jitter", add = TRUE, pch = 21, col = 'black', bg='gray', at=c(1,2,4,5))
}
...
legend(x='topleft',legend=c('GBM','Control'), pt.bg=c('grey','lightblue'), col='black', pch=22, cex=1, pt.cex=2, title="Desease state")
...
lost_sex_DMPs_interrogation = function(cases, controls, lost_DMPs_overlapping_genes_filename, lost_DMPs_overlapping_TFs_filename, TF_names) {
...
mm=match(loss1$name,TF_names)
num_TF_DMPs=length(which(!is.na(mm)))

# Regarding the queston above, how many unique TFs are there?
loss2=loss1[-which(is.na(mm)),]
mm=match(loss2$name,TF_names)
num_TFs=length(unique(TF_names[mm]))
...
if(nrow(loss2 != 0)){
	write.csv(loss2, file=lost_DMPs_overlapping_TFs_filename, row.names=FALSE) # lost DMPs which overlap TFs
}
...
TF_names=as.vector(TFs$HGNC_symbol)
...
lost_DMPs_overlapping_TFs_filename='/athena/masonlab/scratch/users/nai2008/DNAm/second_round_of_scripts/data_tables/lost_DMPs_overlapping_TFs_0to6.csv'
...
lost_sex_DMPs_interrogation(cases=cases, controls=controls, lost_DMPs_overlapping_genes_filename=lost_DMPs_overlapping_genes_filename, 
	lost_DMPs_overlapping_TFs_filename=lost_DMPs_overlapping_TFs_filename, TF_names=TF_names)
...
lost_DMPs_overlapping_TFs_filename='/athena/masonlab/scratch/users/nai2008/DNAm/second_round_of_scripts/data_tables/lost_DMPs_overlapping_TFs_6to21.csv'
...
lost_DMPs_overlapping_TFs_filename='/athena/masonlab/scratch/users/nai2008/DNAm/second_round_of_scripts/data_tables/lost_DMPs_overlapping_TFs_0to21.csv'
...
lost_DMPs_overlapping_TFs_filename='/athena/masonlab/scratch/users/nai2008/DNAm/second_round_of_scripts/data_tables/lost_DMPs_overlapping_TFs_adults.csv'
